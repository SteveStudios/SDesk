...
rm -rf /usr/bin/setup-final.sh

if [ ! -f "/usr/bin/setup-final.sh" ]; then
	chmod -R 755 /etc/profile.d
	chmod -R 777 /usr/share/gnome-shell/extensions

	systemctl stop sdesk-welcome.service
	systemctl disable sdesk-welcome.service

	systemctl daemon-reload

	rm -rf /etc/systemd/system/sdesk-welcome.service
fi